...
async function getPopularProducts(): Promise<StorefrontProductCard[]> {
  ...
  const { data: ranking, error } = await admin.rpc(
  ...
  const [{ data: products }, { data: variants }, { data: subcategories }] =
    await Promise.all([
      admin
        .from("products")
    ...
    return [
      {
        ...
        subcategoryName: product.subcategory_id
          ? (subcategoryNames.get(product.subcategory_id) ?? null)
          : null,
        startingPriceUsd: variantPrices.length
...
export async function getSubcategoryBySlug(slug: string) {
  ...
  const productCards: StorefrontProductCard[] = (products ?? []).map((product) => {
    ...
    return {
      ...
      startingPriceUsd: variantPrices.length
        ? Math.min(...variantPrices)
        : numberValue(product.base_price_usd),
    };
  });
...
export async function getCatalogProductBySlug(
  ...
  const { data: product } = await client
    .from("products")
    .select(
      "id, name, slug, description, image_url, subcategory_id, suboptions_required, base_price_usd, profit_margin_override",
    )
    .eq("slug", slug)
    .eq("status", ACTIVE)
  ...
  const [subcategoryResult, variantsResult, fieldsResult, settingsResult] =
    await Promise.all([
      client
        .from("subcategories")
      ...
      client
        ...
        .select(
          "id, field_key, label, input_type, placeholder, is_required, min_length, max_length, sort_order",
        )
        .eq("product_id", product.id)
        .eq("status", ACTIVE)
        .order("sort_order", { ascending: true }),
  ...
  const margin =
    numberValue(product.profit_margin_override) ??
    numberValue(settingsResult.data?.default_profit_margin) ??
    0;
    ...
    variants: variants.map((variant) => {
      ...
      return {
        ...
        suboptions: suboptions
          .filter((suboption) => suboption.variant_id === variant.id)
          .map((suboption) => {
